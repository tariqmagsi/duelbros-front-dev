...
import UsersRoleRow from './UserRoleRow';
...
export default function CustomPaginationActionsTable({ data, columns, role, type, handleOpen, handleClose, open, loading, loadingDltButton, update, deleteRow }) {
    ...
    const handleSelectedRow = (type, _data) => {
        if (type === 'player') {
            ...
            staticData = _data;
        } else if (type === 'user' || type === 'user_role') {
                            ...
                            role === "player" ? <PlayersRow key={i} row={row} handleOpen={handleOpen} setSelectedRow={handleSelectedRow.bind(this, 'player')} /> :
                                role === "user" ? <UsersRow key={i} row={row} handleOpen={handleOpen} setSelectedRow={handleSelectedRow.bind(this, 'user')} /> :
                                    role === "moderator" ? <ModeratorsRow key={i} row={row} handleOpen={handleOpen} setSelectedRow={handleSelectedRow.bind(this, 'moderator')} /> :
                                        role === "user_role" ? <UsersRoleRow key={i} row={row} handleOpen={handleOpen} setSelectedRow={handleSelectedRow.bind(this, 'user_role')} /> :
                                            <TableRow key={i} />
                        ))}
